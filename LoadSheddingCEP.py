...
class LoadSheddingCEP(CEP):
    ...
    def _apply_load_shedding_to_tree(self):
        ...
        if not hasattr(eval_mechanism, '_tree_plan'):
            return
            
        tree_plan = eval_mechanism._tree_plan
        
        # Get all nodes from the tree plan
        all_nodes = []
        ...
        print(f"🔧 Load Shedding Applied: Wrapped {self.load_shedding_stats['total_storages_wrapped']} storage units")
    ...
    def _collect_load_shedding_stats(self):
        ...
        # Get all nodes again to collect stats
        if not hasattr(self, '_CEP__evaluation_manager'):
            return
    ...
    def _report_load_shedding_results(self, processing_time: float, wall_time: float):
        ...
        print("\n" + "=" * 60)
        print("🛡️  LOAD SHEDDING RESULTS")
        print("=" * 60)
        
        if not self.load_shedding_config.enabled:
            print("❌ Load shedding was DISABLED")
            return
            
        print(f"✅ Load shedding was ENABLED")
        print(f"📦 Storage nodes wrapped: {stats['total_storages_wrapped']}")
        print(f"🗑️  Total partial matches dropped: {stats['total_dropped_matches']}")
        print(f"⚡ Load shedding events: {stats['shedding_events']}")
        print(f"💾 Final storage size: {stats.get('final_storage_size', 0)}")
        
        if stats['total_dropped_matches'] > 0:
            print(f"📈 Load shedding successfully prevented storage explosion!")
            print(f"🎯 Strategy '{self.load_shedding_config.shedding_strategy}' maintained bounded latency")
        else:
            print(f"ℹ️  No load shedding was needed (storage stayed within limits)")
